...
class EncounterList_Scene
  ...
  def drawPresent
    ...
    loctext += sprintf("<ac><c2=7FF05EE8>Total encounters for area: %s</c2></ac>",enc_array.length)
  ...
  def drawAbsent
    loctext = _INTL("<ac><b><c2=43F022E8>{1}</b></c2></ac>", $game_map.name)
    loctext += sprintf("<ac><c2=7FF05EE8>This area has no encounters!</c2></ac>")
    #loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
    @sprites["locwindow"].setText(loctext)
  end
