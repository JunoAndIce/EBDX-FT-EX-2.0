...
class ItemStorage_Scene
  # Changed Colors
  ITEMLISTBASECOLOR   = Color.new(90, 82, 82)
  ITEMLISTSHADOWCOLOR = Color.new(165, 165, 173)
  ITEMTEXTBASECOLOR   = Color.new(255, 255, 255)
  ITEMTEXTSHADOWCOLOR = Color.new(82, 82, 82)
  TITLEBASECOLOR      = Color.new(90, 82, 82)
  TITLESHADOWCOLOR    = Color.new(165, 165, 173)
  # Changed number of items in the list
  ITEMSVISIBLE        = 6
  ...
  def pbStartScene(bag, party)
    ...
    @sprites["icon"] = ItemIconSprite.new(50, 332, nil, @viewport)
    # Item list
    # Changed list position
    @sprites["itemwindow"] = Window_PokemonItemStorage.new(@bag, 98, 60, 334, 32 + (ITEMSVISIBLE * 32))
    ...
    # Title
    @sprites["pocketwindow"] = BitmapSprite.new(176, 32, @viewport)
    @sprites["pocketwindow"].x = 14
    @sprites["pocketwindow"].y = 16
    pbSetNarrowFont(@sprites["pocketwindow"].bitmap)
    # Item description
    # Changed item description position
    @sprites["itemtextwindow"] = Window_UnformattedTextPokemon.newWithSize("", 86, 262, Graphics.width - 84, 128, @viewport)
  ...
  def pbRefresh
    ...
    drawTextEx(bm, 0, 4, bm.width, 2, @title, ITEMTEXTBASECOLOR, ITEMTEXTSHADOWCOLOR)
