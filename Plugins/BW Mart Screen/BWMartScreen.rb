...
class PokemonMart_Scene
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    @sprites["icon"] = ItemIconSprite.new(34, Graphics.height - 50, nil, @viewport)
    ...
    @sprites["itemtextwindow"].shadowColor = Color.new(165, 165, 173)
    ...
    @sprites["moneywindow"].setSkin("Graphics/Windowskins/moneywindow")
    ...
    @sprites["moneywindow"].baseColor = Color.new(255, 255, 255)
    @sprites["moneywindow"].shadowColor = Color.new(165, 165, 173)
    @sprites["qtywindow"] = Window_AdvancedTextPokemon.new("")
    pbPrepareWindow(@sprites["qtywindow"])
    @sprites["qtywindow"].setSkin("Graphics/Windowskins/bw choice")
  ...
  def pbStartSellScene2(bag, adapter)
    ...
    @subscene.pbStartScene(bag, $player.party) #FT
    @viewport = Viewport.new(0, 0, Graphics.width, Graphics.height)
    @viewport.z = 99999
    @sprites = {}
    @sprites["helpwindow"] = Window_AdvancedTextPokemon.new("")
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.width = 224
      numwindow.height = 64
      numwindow.baseColor = Color.new(248, 248, 248) # Color.new(88, 88, 80)
      numwindow.shadowColor = Color.new(72, 80, 88) # Color.new(168, 184, 184)
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  if $game_switches[Settings::OCEANE_SHOP]
	pbCallBub(2,11)
    cmd = pbMessage(speech || _INTL("<b>Madamé Oceane</b>: I've got them all. What do you need?"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::TR_SHOP]
    cmd = pbMessage(speech || _INTL("Booting up TR Distributor."), commands, cmdQuit + 1)
  elsif $game_switches[Settings::FUJISAKI_SHOP]
	pbCallBub(2,3)
    cmd = pbMessage(speech || _INTL("<b>Sara</b>: Glad to have you here! What can I supply you with?"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::ANGRY_SHOP]
	pbCallBub(2,5)
    cmd = pbMessage(speech || _INTL("You buying something or what...?"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::CAFE_SHOP]
	pbCallBub(2,8)
    cmd = pbMessage(speech || _INTL("So what you gettin'? What you buyin'?"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::CELEBI_SHOP]
	pbCallBub(2,5)
    cmd = pbMessage(speech || _INTL("Kyuuuuuun!"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::RAMEN_SHOP]
	talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
    pbCallBub(2, talker)
    cmd = pbMessage(speech || _INTL("We got a small but <b>powerful</b> selection!"), commands, cmdQuit + 1)
  elsif $game_switches[Settings::FLORAL_SHOP]
	talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
    pbCallBub(2, talker)
    cmd = pbMessage(speech || _INTL("We have all your gardening needs!"), commands, cmdQuit + 1)
  else
	pbCallBub(2,5)
  ...
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      ...
      if $game_switches[Settings::OCEANE_SHOP]
        pbCallBub(2,11)
        pbMessage(_INTL("<b>Madamé Oceane</b>: Godspeed, trainer."))
        break
      elsif $game_switches[Settings::TR_SHOP]
        pbMessage(_INTL("TR Distributor shutting down..."))
        break
      elsif $game_switches[Settings::FUJISAKI_SHOP]
        pbCallBub(2,3)
        pbMessage(_INTL("<b>Sara</b>: We hope to have your patronage again, \PN."))
        break
      elsif $game_switches[Settings::ANGRY_SHOP]
        pbCallBub(2,5)
        pbMessage(_INTL("I need to get out of here..."))
        break
      elsif $game_switches[Settings::CAFE_SHOP]
        pbCallBub(2,8)
        pbMessage(_INTL("Alright then! Enjoy the food, eh? "))
        break
      elsif $game_switches[Settings::CELEBI_SHOP]
        pbCallBub(2,5)
        pbMessage(_INTL("Kyuuuun!"))
        break
      elsif $game_switches[Settings::RAMEN_SHOP]
		talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
        pbCallBub(2, talker)
        pbMessage(_INTL("Enjoy your Ramen! We hope to see you again!"))
        break
      elsif $game_switches[Settings::FLORAL_SHOP]
		talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
        pbCallBub(2, talker)
        pbMessage(_INTL("We hope you bring beauty to the world with your gardening!"))
        break
      else
        pbCallBub(2,5)
    ...
    if $game_switches[Settings::OCEANE_SHOP]
        pbCallBub(2,11)
        cmd = pbMessage(_INTL("<b>Madamé Oceane</b>: Do you need anything else? Don't waste time, now."), commands, cmdQuit + 1)
    elsif $game_switches[Settings::TR_SHOP]
        cmd = pbMessage(_INTL("<b>TR Distributor</b> up and running at full capacity."), commands, cmdQuit + 1)
    elsif $game_switches[Settings::FUJISAKI_SHOP]
        pbCallBub(2,3)
        cmd = pbMessage(_INTL("<b>Sara</b>: Will that be all today?"), commands, cmdQuit + 1)
     elsif $game_switches[Settings::ANGRY_SHOP]
        pbCallBub(2,5)
        cmd = pbMessage(_INTL("Can you like... Hurry this up?"), commands, cmdQuit + 1)
    elsif $game_switches[Settings::CAFE_SHOP]
        pbCallBub(2,8)
        cmd = pbMessage(_INTL("Anything else I cant get for ya'?"), commands, cmdQuit + 1)
    elsif $game_switches[Settings::CELEBI_SHOP]
        pbCallBub(2,5)
        cmd = pbMessage(_INTL("Kyuun?"), commands, cmdQuit + 1)
    elsif $game_switches[Settings::RAMEN_SHOP]
        talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
        pbCallBub(2, talker)
        cmd = pbMessage(_INTL("Anything else I can do for ya'?"), commands, cmdQuit + 1)
    elsif $game_switches[Settings::FLORAL_SHOP]
        talker = $game_variables[Settings::RAMEN_SHOP_TALKER]
        pbCallBub(2, talker)
        cmd = pbMessage(_INTL("Are you looking to get anything else?"), commands, cmdQuit + 1)
      else
        pbCallBub(2,5)
