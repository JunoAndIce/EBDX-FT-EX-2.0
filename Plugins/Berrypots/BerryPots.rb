  ...
  def pbBerryPlantVar(idx)
    ...
    if Settings::NEW_BERRY_PLANTS
      ...
      if pbConfirmMessage(_INTL("It's soft, loamy soil.\nPlant a berry?"))
        ...
        return if !CAN_PLANT_APRICORN
        return if !pbConfirmMessage(_INTL("It's soft, loamy soil.\nPlant an apricorn?"))
        choose = :plantapricorn
        ask_to_plant = false
      end
    end
    if (!ask_to_plant || pbConfirmMessage(_INTL("Want to plant a Berry?"))) && choose != :plantapricorn
      if !hasBerryItem
    ...
    elsif choose == :plantapricorn && (!ask_to_plant || pbConfirmMessage(_INTL("Want to plant an Apricorn?")))
      if !hasApricorn
      ...
      pbFadeOutIn {
        scene = PokemonBag_Scene.new
        screen = PokemonBagScreen.new(scene, $bag)
        berry = screen.pbChooseItemScreen(proc { |item| GameData::Item.get(item).is_apricorn? })
      }
      if berry
        $stats.berries_planted += 1
        berry_plant.plant(berry)
